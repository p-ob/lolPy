...
class Client(object):
    ...
    def change_search_parameters(self, username: str=None, region: str=None):
        if (username and username != self.__name) or (region and region != self.__region):
            self.__name = username if username else self.__name
            self.__region = region if region else self.__region
            self.__search_for_player()
    ...
    def ranked_stats(self):
        if self.__player is None:
            return []
        payload = {"api_key": self.__key}
        url = (urls.base + urls.ranked_stats).format(region=self.__region, summonerId=self.__player.id)
        r = requests.get(url, params=payload)
        if r.status_code == RiotException.RateLimitExceeded:
            time.sleep(1)
    ...
    def summary_stats(self):
        if self.__player is None:
            return []
        payload = {"api_key": self.__key}
        url = (urls.base + urls.general_stats).format(region=self.__region, summonerId=self.__player.id)
        r = requests.get(url, params=payload)
        if r.status_code == RiotException.RateLimitExceeded:
            time.sleep(1)
            return self.ranked_stats()
